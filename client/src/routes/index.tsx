...
export default component$(() => {
  ...
  return (
    <div class="font-display bg-background-light min-h-screen flex items-center justify-center p-4">
      {/* Subtle Gradient Background Elements */}
      <div class="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div class="bg-blob-1 absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-primary/10 blur-[120px] opacity-0" />
        <div class="bg-blob-2 absolute -bottom-[10%] -right-[10%] w-[40%] h-[40%] rounded-full bg-primary/10 blur-[120px] opacity-0" />
      </div>

      {/* Main Auth Card */}
      <div class="auth-card relative z-10 w-full max-w-[440px] bg-white shadow-xl rounded-xl border border-primary/10 overflow-hidden opacity-0">
        {/* Header / Logo Section */}
        <div class="px-8 pt-10 pb-6 text-center">
          <div class="flex flex-col items-center gap-3">
            <div class="size-16 bg-primary/10 text-primary rounded-xl flex items-center justify-center border border-primary/20">
      ...
      <div class="fixed bottom-6 right-6 hidden md:flex items-center gap-3 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full border border-primary/10 shadow-sm">
        <div class="size-2 rounded-full bg-secondary shadow-[0_0_8px_rgba(250,204,21,0.6)]"></div>
        <p class="text-xs font-medium text-slate-600">Sistem Berjalan Normal</p>
        <div class="w-[1px] h-4 bg-slate-200 mx-1"></div>
        <button class="text-xs font-bold text-primary hover:text-blue-700 transition-colors">Bantuan</button>
      </div>
    </div>
  );
});
